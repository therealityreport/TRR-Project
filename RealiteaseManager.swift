...
class RealiteaseManager: ObservableObject {
                        ...
                        if guessedInfo.CastName == self.correctCastInfo?.CastName {
                            self.updateUserStats(for: date, result: .won) {
                                self.updateMainDocumentStats(uid: self.userId!, result: .won) {
                                    self.updateGuessDistribution(for: newGuess.guessNumber) {
                                        self.fetchUserStats {
                                            completion(.won)
                                        }
                                    }
    ...
    private func updateGuessDistribution(for guessNumber: Int, completion: @escaping () -> Void) {
        guard let userId = userId else { return }
        let guessDistributionRef = db.collection("user_analytics").document(userId).collection("realitease_guessDistribution").document("\(guessNumber)")
        
        guessDistributionRef.getDocument { document, error in
            if let document = document, document.exists {
                guessDistributionRef.updateData([
                    "guessCount": FieldValue.increment(Int64(1))
                ]) { error in
                    if let error = error {
                        print("Failed to update guess distribution: \(error)")
                    }
                    completion()
                }
            } else {
                guessDistributionRef.setData([
                    "guessCount": 1
                ]) { error in
                    if let error = error {
                        print("Failed to create guess distribution: \(error)")
                    }
                    completion()
                }
            }
    ...
    private func updateAnalyticsForDate(date: Date, guessNumberSolved: Int, didWin: Bool) {
        let formatter = DateFormatter()
        formatter.dateFormat = "MM-dd-yyyy"
        let dateString = formatter.string(from: date)
        ...
        analyticsRef.getDocument { document, error in
            if let document = document, document.exists {
                var totalAttempts = document.data()?["totalAttempts"] as? Int ?? 0
                var totalWins = document.data()?["totalWins"] as? Int ?? 0
                var averageGuesses = document.data()?["averageGuesses"] as? Double ?? 0.0
                var guessDistribution = document.data()?["guessDistribution"] as? [String: [String: Int]] ?? [:]
                ...
                for (index, guess) in self.guesses.enumerated() {
                    let guessNumber = index + 1
                    let guessKey = "attempt\(guessNumber)"
                    let guessName = guess.guessedInfo.CastName
                    
                    if var guessCounts = guessDistribution[guessKey] {
                        guessCounts[guessName, default: 0] += 1
                        guessDistribution[guessKey] = guessCounts
                    } else {
                        guessDistribution[guessKey] = [guessName: 1]
                    }
                }
                
                averageGuesses = ((averageGuesses * Double(totalAttempts - 1)) + Double(guessNumberSolved)) / Double(totalAttempts)
                ...
                let initialData: [String: Any] = [
                    ...
                    "averageGuesses": Double(guessNumberSolved),
                    "guessDistribution": self.guesses.enumerated().reduce(into: [:]) { dict, tuple in
                        let (index, guess) = tuple
                        let guessKey = "attempt\(index + 1)"
                        dict[guessKey] = [guess.guessedInfo.CastName: 1]
                    }
                ]
                analyticsRef.setData(initialData) { error in
                    if let error = error {
                        print("Failed to create initial analytics: \(error)")
                    }
                }
            }
        }
